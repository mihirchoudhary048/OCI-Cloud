...
def get_lpgs_for_compartment(compartment_id, region):
    """Retrieve all Local Peering Gateway details for a given compartment in a specific region."""
    command = f"oci network local-peering-gateway list --compartment-id {compartment_id} --region {region} --all --output json"
    return run_oci_command(command)
...
def collect_lpg_data(output_dir):
    ...
    for region in regions:
        for compartment in compartments.get('data', []):
            ...
            print(f"Processing LPGs for compartment: {compartment_name} (ID: {compartment_id}) in region: {region}")

            lpgs = get_lpgs_for_compartment(compartment_id, region)
            if lpgs:
                save_json_to_file(output_dir, f"{region}_{compartment_name}_lpgs.json", lpgs)

if __name__ == "__main__":
    output_directory = sys.argv[1]
    collect_lpg_data(output_directory)
